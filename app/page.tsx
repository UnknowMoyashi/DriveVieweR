...
const FileThumbnail = ({ src, isVideo, isFolder }: { src?: string; isVideo?: boolean; isFolder?: boolean }) => {
  ...
  if (!src) return (
    <div className="w-full h-full flex items-center justify-center bg-gray-800/50">
      <span className={isFolder ? "text-6xl" : "text-4xl opacity-20"}>{isFolder ? "📂" : "📄"}</span>
    </div>
  );
    ...
    <div className="relative w-full h-full bg-gray-800">
      ...
      <img
        ...
        className={`w-full h-full object-cover transition-opacity duration-700 ${
          isLoaded ? (isFolder ? "opacity-60 group-hover:opacity-100" : "opacity-100") : "opacity-0"
        }`}
      />
      {isVideo && isLoaded && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/20">
          <div className="bg-white/10 p-3 rounded-full backdrop-blur-sm text-2xl">▶️</div>
        </div>
      )}
      {isFolder && <div className="absolute top-3 left-3 z-10 text-3xl drop-shadow-lg">📂</div>}
    </div>
  );
};
...
export default function Home() {
  ...
  const [cache, setCache] = useState<Record<string, DriveFile[]>>({});

  // --- 1. 初期化 & 履歴監視 ---
  useEffect(() => {
    // キャッシュ読み込み
    const savedCache = localStorage.getItem("drive_cache");
    if (savedCache) {
      try { setCache(JSON.parse(savedCache)); } catch (e) {}
    }

    // 初期フォルダ判定
    const params = new URLSearchParams(window.location.search);
    const folderIdFromUrl = params.get("f") || "root";
    setCurrentFolderId(folderIdFromUrl);

    // ブラウザの戻る・進むを監視
    const handlePopState = (event: PopStateEvent) => {
      // モーダルが開いている状態で「戻る」が押された場合
      // (Stateにmodal: trueが入っていない状態に戻ったらモーダルを閉じる)
  ...
  useEffect(() => {
    if (Object.keys(cache).length > 0) {
      localStorage.setItem("drive_cache", JSON.stringify(cache));
    }
  }, [cache]);
  ...
  useEffect(() => {
    if (!session) return;
    if (cache[currentFolderId]) setFiles(cache[currentFolderId]);
    ...
    fetch(`/api/drive?folderId=${currentFolderId}`)
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          setFiles(data);
          setCache((prev) => ({ ...prev, [currentFolderId]: data }));
        }
        setLoading(false);
      })
  ...
  const handleFolderClick = (id: string) => {
    window.history.pushState({ folderId: id, modal: false }, "", `?f=${id}`);
  ...
  const handleFileClick = (file: DriveFile) => {
    // 履歴に「モーダル表示中」というフラグを積む
    window.history.pushState({ folderId: currentFolderId, modal: true }, "", window.location.search);
  ...
  const closeModal = () => {
    if (selectedFile) {
      // 手動で閉じる場合は、ブラウザの履歴を1つ戻す（これでpopstateが走りsetSelectedFile(null)になる）
      window.history.back();
    }
  };
  ...
  return (
    <div className="p-4 bg-gray-950 min-h-screen text-gray-100">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            {currentFolderId !== "root" && (
              <button 
                ...
                className="bg-gray-800 hover:bg-gray-700 px-4 py-1 rounded-lg text-sm transition"
              >
                ← 戻る
              </button>
            )}
        ...
        {(loading && !cache[currentFolderId]) ? (
          <div className="flex flex-col justify-center items-center py-40 gap-4">
             <div className="animate-spin h-10 w-10 border-4 border-blue-500 border-t-transparent rounded-full"></div>
             <span className="text-gray-500 font-medium">取得中...</span>
          </div>
        ) : (
            ...
            {files.map((file) => {
              ...
              return (
                <div 
                  ...
                  onClick={() => isFolder ? handleFolderClick(file.id) : handleFileClick(file)}
                >
                  ...
                  <div className="p-3 bg-gray-900/80 backdrop-blur-sm">
                    <p className="text-[11px] text-center truncate text-gray-400 group-hover:text-white transition-colors font-medium">
                      {file.name}
                    </p>
                  </div>
                </div>
      ...
      {/* プレビューモーダル */}
      {selectedFile && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 p-4 transition-all backdrop-blur-sm" onClick={closeModal}>
          ...
          <div className="max-w-5xl w-full h-full flex flex-col items-center justify-center animate-in zoom-in duration-300" onClick={(e) => e.stopPropagation()}>
            {selectedFile.mimeType.startsWith("image/") ? (
              ...
              <div className="text-center bg-gray-900 p-12 rounded-3xl border border-gray-800 text-white">
                <p className="mb-8 text-xl">プレビュー不可</p>
                <a href={selectedFile.webContentLink} className="bg-blue-600 px-8 py-3 rounded-full font-bold">ダウンロード</a>
              </div>
            )}
